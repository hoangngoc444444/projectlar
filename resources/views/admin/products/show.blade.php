                ...
                <div class="panel-body">
                    <form action="{{ route('admin.product.update',['id' => $product->id]) }}" method="POST"
                        ...
                        <div class="form-group {{ $errors->has('regular_price') ? 'has-error' : '' }}">
                            ...
                            <input type="number" min="1" class="form-control" id="regular_price" name="regular_price"
                                placeholder="Giá thị trường" value="{{ $product->regular_price }}">
                        ...
                        <div class="form-group {{ $errors->has('sale_price') ? 'has-error' : '' }}">
                            ...
                            <input type="number" min="0" class="form-control" id="sale_price" name="sale_price"
                                placeholder="Giá bán" value="{{ $product->sale_price }}">
                        ...
                        <div class="form-group {{ $errors->has('original_price') ? 'has-error' : '' }}">
                            ...
                            <input type="number" min="0" class="form-control" id="original_price" name="original_price"
                                placeholder="Giá gốc" value="{{ $product->original_price }}">
                        ...
                        <div class="form-group {{ $errors->has('images.*') ? 'has-error' : '' }}">
                            ...
                            <label for="images">Thư viện ảnh sản phẩm</label>
                           
                            <div>
                                @foreach ($product->attachments as $file)
                                @if (file_exists(public_path(get_thumbnail("uploads/$file->path"))))
                                <img src="{{ asset(get_thumbnail("uploads/$file->path")) }}" alt="images"
                                    class="img-responsive img-thumbnail">
                                @else
                                <img src="{{ asset('images/no_image.jpg') }}" alt="No images"
                                    class="img-responsive img-thumbnail">
                                @endif
                                @endforeach
                            </div>
...
<script type="text/javascript">
    @php
        $attributes = $product->attributes ? json_encode($product->attributes) : null;
        @endphp
